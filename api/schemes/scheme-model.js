...
function find() { // EXERCISE A
  /*
    ...
    A. The 7th entry is ommitted as it has no steps to be counted (no steps means no matching row in steps table)
      SELECT
          sc.*,
...
async function findById(id) { // EXERCISE B
  ...
  */
      ...
      // First structure the database query
      /* SELECT
          sc.scheme_name,
          st.*
      FROM schemes as sc
      LEFT JOIN steps as st
          ON sc.scheme_id = st.scheme_id
      WHERE sc.scheme_id = 1 --id must be dynamically sourced in actual code
      ORDER BY st.step_number ASC;
   */
